...
from mateditor.export import BINARIES_PATH, BLCMExport
...
def draw_save_modal() -> None:
    ...
    imgui.text(
        f"Will be saved as '{(BINARIES_PATH / f'../{BLCMExport.file_name}').resolve()}.blcm'",
    )
    ...
    if imgui.button("Save"):
        if Materials.selected:
            BLCMExport.save_to_file(
                BINARIES_PATH / f"../{BLCMExport.file_name}.blcm",
...
def draw_texture_parameter_popup() -> None:
    selected_material = cast(MaterialInstanceConstantProxy, Materials.selected)
    ...
    changed, Texture2D.index = imgui.list_box(
        "##TextureListBox",
        Texture2D.index,
        textures,
        16,
    ...
    if changed:
        selected_material.set_texture_parameter_value(
            Texture2D.texture_parameter,
            textures[Texture2D.index],
        )
