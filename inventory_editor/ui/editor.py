...
from inventory_editor.backpack import ItemData, WeaponData, player_inventory

from .components import combo_with_title


def part_changed(part_name: str, value: unreal.UObject|None) -> None:
    player_inventory.on_part_changed(part_name, value)


@lru_cache(maxsize=32)
def readable_list(items: tuple[unreal.UObject, ...]) -> list[str]:
...
def parts_combo(
    part_name: str,
    current: int,
    parts_uobject: tuple[unreal.UObject|None, ...],
) -> int:
    changed: bool
...
def game_stage_slider(attr: str, value: int) -> int:
    ...
    if changed and player_inventory.item:
...
def draw_weapon_editor() -> None:
    ...
    weapon_data.ManufacturerGradeIndex = game_stage_slider("ManufacturerGradeIndex", weapon_data.ManufacturerGradeIndex)
...
def draw_item_editor() -> None:
    ...
    item_data.ItemDefinition_Index = parts_combo(
        "ItemDefinition",
        item_data.ItemDefinition_Index,
        item_data.ItemDefinition,
    )
    ...
    item_data.ManufacturerGradeIndex = game_stage_slider("ManufacturerGradeIndex", item_data.ManufacturerGradeIndex)
    ...
    imgui.text("Quantity:")
    imgui.push_item_width(-1)
    if player_inventory.item:
        changed, player_inventory.item.Quantity = imgui.slider_int("##Quantity", player_inventory.item.Quantity, 1, 999)
    else:
        _1, _2 = imgui.slider_int("##Quantity", 1, 1, 999)
