...
PauseWhile = WaitWhile | WaitUntil

__all__ = ["Tween"]


GameIsPaused: WaitWhile = WaitWhile(lambda: get_pc().IsPaused())


def step_tuple(tup_start: tuple, tup_end: tuple, t: float) -> tuple:
    """Step a tuple from start to end by the given t value"""
    return tuple(start + (end - start) * t for start, end in zip(tup_start, tup_end, strict=False))
...
class Tweener:
    ...
    def __init__(self, final_value: Any, duration: float) -> None:
        ...
        self._step_function: Callable[[Any, Any, float], Any] = CUSTOM_STEPS.get(type(final_value), step_float)
...
class PropertyTweener(Tweener):
    ...
    def __init__(
        ...
        self._step_function = CUSTOM_STEPS.get(type(getattr(obj, property_name)), step_float)
...
class Tween:
    def __init__(self) -> None:
        ...
        self._pause_while: PauseWhile | None = GameIsPaused
        self._tweeners: list[Tweener] = []
        self._last_tweener: Tweener | None = None

    def tween_property(self, obj: Any, property_name: str, final_value: Any, duration: float) -> PropertyTweener:
    ...
    def pause_while(self, condition: PauseWhile | None) -> Tween:
        ...
        """
        if self._started:
            raise RuntimeError("Cannot change pause condition on a started Tween object")
